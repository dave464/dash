$(document).ready(function() {
  ...
  measurementId: "G-3Z6R95RYGC"
    ...
    var Waterstation = db.ref().child("adminFeedback");
...
$('.edit').click(function(){
    ...
    Waterstation.on("child_added", data => {        
        dataSet = [data.key, data.child("Name").val(), data.child("date").val(), data.child("comment").val()];
    ...
    $("#tableInspection").on("click", ".btnDelete", function() {   
        ...
        if (result.value) {
            let row = $('#tableInspection').dataTable().fnGetData($(this).closest('tr'));            
            let id = row[0];            
            db.ref(`adminFeedback/${id}`).remove()
