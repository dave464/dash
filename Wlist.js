$(document).ready(function() {
  ...
  measurementId: "G-3Z6R95RYGC"
    ...
    var rowEliminated; //para capturara la fila eliminada
    var rowEdited; //para capturara la fila editada o actualizada
                ...
                columnDefs: [
                    {
                        ...
                        visible: false,
                         //ocultamos la columna de ID que es la [0]                        
                    },
                    {
                        targets: -1,        
                        defaultContent: "<div class='wrapper text-center'><div class='btn-group'><button class='btnEdit btn btn-primary' data-toggle='tooltip' title='Edit'>"+iconEdit+"</button><button class='btnDelete btn btn-danger' data-toggle='tooltip' title='Delete'>"+iconClear+"</button></div></div>"
                    }
                ],
...
.once("value", function (snapshot) {
  ...
  document.getElementById("banner").innerHTML = count;
});


    var ref = firebase.database().ref("inspection");
    let state = 0;
   const count3 = ref
        .orderByChild("Status")
        .equalTo("Passed")
        .on("child_added", function (snapshot) {
            const count2 = [];
            var count1 = snapshot.val().Address;

            count2.push(count1);
            console.log(count2);

            state = state + 1;
            document.getElementById("banner1").innerHTML = state;
        });
...
.on("child_added", function (snapshot) {
  ...
  const passed2 = [];
  var passed1 = snapshot.val().Status;
...
.on("child_added", function (snapshot) {
  ...
  const count6 = [];
  var count5 = snapshot.val().Status;
